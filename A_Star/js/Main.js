	...
	for (var i = 0; i < this.currentNode.neighbourList.length; i++) {
		...
		if (openList.includes(neighbourNode)) {
			if (newTotalCost < neighbourNode.totalCost) {
				...
				neighbourNode.finalCost = 8*neighbourNode.heuristic + neighbourNode.totalCost;
			}
			continue;
		}
		...
		if (closedList.includes(neighbourNode)) {
			if (newTotalCost < neighbourNode.totalCost) {
				...
				closedList.splice(closedList.indexOf(neighbourNode),1);
			}
			continue;
		}
...
function visualize(Grid) {
	...
	game.add.sprite( 175 + 45* openList[0].x, 50 + 45 *openList[0].y, 'state', 2);
}
function preload() {
...
function render() {
	//game.debug.text(Grid.getNode(2,5).givenCost,175 + 45* 2, 50 + 45 * 5);
	visualizeCost(Grid);
